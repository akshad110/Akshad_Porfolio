...
export function CursorGrid({
  ...
  useEffect(() => {
    ...
    const isBackgroundHit = (event: PointerEvent) => {
      const target = document.elementFromPoint(event.clientX, event.clientY);
      if (!(target instanceof Element)) return false;
      if (excludeSelector && target.closest(excludeSelector)) return false;
      if (
        target.closest(
          "header, nav, footer, article, button, a, input, textarea, select, img, video, svg, [data-grid-ignore], [role='dialog']",
        )
      ) {
        return false;
      }
    ...
    const onPointerMove = (event: PointerEvent) => {
      if (!isBackgroundHit(event)) return;
      energize(...toLocal(event));
      wake();
    };

    const onPointerDown = (event: PointerEvent) => {
      if (!propsRef.current.clickPulse || !isBackgroundHit(event)) return;
